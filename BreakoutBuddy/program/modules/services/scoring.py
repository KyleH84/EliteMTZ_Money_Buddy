...
def _ensure_rank_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Fill missing core columns with defaults
    ...
    # Try agent-based enrichment if available
    try:
    ...
    # --- NEW: run our enrichment to replace "None" with real values ---
    try:
...
def _persist_ranked(df: pd.DataFrame) -> None:
    """Persist latest ranked table to disk (legacy, but harmless in cloud)."""
    try:
...
def rank_now(arg) -> pd.DataFrame | tuple:
    """
    Rank stocks now.
    - If arg is a DataFrame, rank directly.
    - Else, treat arg as settings dict (with universe_size, top_n).
    Returns either a DataFrame or a tuple (snap, regime, ranked, None, None).
    """
