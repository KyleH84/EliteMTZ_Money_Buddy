# streamlit_app.py — cloud-friendly launcher with logs, BAT→Direct fallback, cleaner, and Recreate .venv
from __future__ import annotations
import os, sys, time, socket, subprocess, webbrowser, shutil
from pathlib import Path
import streamlit as st

st.set_page_config(page_title="Money Buddy — Home (Cloud)", page_icon="💼", layout="wide")
...
def _recreate_venv_for_app(name: str, cfg: dict, log_dir: Path | None) -> str:
    ...
    rc, _ = _run([sys.executable, "-m", "venv", str(venv_dir)], cwd=app_root, log_file=log_path)
    if rc != 0:
...
# ---------------- UI ----------------
st.title("💼 Money Buddy — Home (Cloud)")
st.write("Start/stop, open, embed, clean junk, or rebuild venvs.")

with st.expander("Launcher options (advanced)", expanded=True):
...
if log_dir:
    ...
    for name in APPS:
        ...
        if lf.exists():
            ...
            st.text_area(f"Log: {name}", txt, height=200, key=f"log_{name}")
    if not found_any:
        st.caption("No logs yet. Start an app with “Capture logs” enabled.")
